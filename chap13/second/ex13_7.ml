...
let print_eki_list eki_list =
  let print_eki eki =
    Printf.printf "namae: %s, saitan_kyori: %f, temae_list: [%s]\n"
      eki.namae
      eki.saitan_kyori
      (String.concat "; " eki.temae_list)
  in
  List.iter print_eki eki_list


let rec koushin p q_list = match q_list with
| [] -> []
| f :: r -> let tmp = koushin1 p f in
  tmp :: koushin p r
;;

(* test: ocaml ex13_7.ml *)
let () = Printf.printf "\nkoushin\n";;
